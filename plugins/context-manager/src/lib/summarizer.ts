...
import { Chunk, messagesToText } from './chunker.js';
...
export interface Summary {
  ...
  level: number; // 0 = original chunks, 1 = first summary, 2 = summary of summaries, etc.
}
...
export class Summarizer {
  ...
  private targetSummaryTokens: number;
   ...
   */
  async summarizeChunk(chunk: Chunk): Promise<Summary> {
...
Provide a comprehensive synthesis in markdown format:`;
   ...
   */
  async recursiveSummarize(chunks: Chunk[], progressCallback?: (status: string) => void): Promise<Summary> {
    if (chunks.length === 0) {
