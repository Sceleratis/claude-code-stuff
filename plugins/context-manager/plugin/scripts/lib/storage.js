 ...
 */
export function getContextDir(projectRoot) {
    return join(projectRoot, '.claude', 'context');
}
/**
 * Ensure context directory exists
 */
export function ensureContextDir(projectRoot) {
 ...
 */
export function formatSummary(summary, metadata) {
    const now = new Date();
 ...
 */
export function saveSummary(projectRoot, summary, description, messageCount, originalTokens, summaryTokens) {
 ...
 */
export function listContextFiles(projectRoot) {
    const contextDir = getContextDir(projectRoot);
    if (!existsSync(contextDir)) {
        return [];
    }
 ...
 */
export function loadContext(filepath) {
    return readFileSync(filepath, 'utf-8');
}
/**
 * Get most recent context file
 */
export function getMostRecentContext(projectRoot) {
